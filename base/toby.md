...
- Run the voice checker on every prose file written this turn, without being asked. It is `scripts/voice-check.py` in the installed `toby-voice` skill folder, such as `~/.codex/skills/toby-voice/scripts/voice-check.py`, `~/.claude/skills/toby-voice/scripts/voice-check.py`, or `~/.copilot/skills/toby-voice/scripts/voice-check.py`. Run it with `python3`. Fix everything it puts under FIX. Read every line under DECIDE and make a decision about each flagged sentence, because most of those flags are correct. When the checker is not on this machine, say plainly that it is missing, and read the files yourself.
