...
A design smell holds three different lines: the entry name from references/smells.md, the file:line, and the code there that meets the entry's Fires-when criterion. A smell costs future readers and future changes, so demanding a runtime consequence of it would drop every real one or invent a consequence for it. The drop rule above still governs — three lines you can fill from the file, or there's no finding.
...
- A design smell matching a named entry in references/smells.md, only when the entry's criteria can be quoted against the code. Method length on its own and a comment's mere presence stay non-findings no matter what the catalog calls them.
