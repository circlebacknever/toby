...
illustrative, but the reasoning applies to other code.
...
Why it causes problems: every caller that formats a name now silently depends on this
...
```python
def display_name(user):
    return user.display_label()      # the User class returns the answer to
...
The design pass took a minute. It was based on a near-future variant ("there will be more
user states"). Now new states go in one place, callers are untouched,
and the formatter no longer checks for "deleted". The interface got
simpler while the work moved into `User`.
...
def process(self, payment):
    # TACTICAL: this gateway switch is hardcoded for ACME only and shipped under the
...
person can see the debt and fix it. Under a deadline, label the shortcut
and state the exit.
...
It works, and then push is added, then Slack, then a webhook. Each one requires an edit to `send`.
Three call sites now have their own copy of the same `if` chain to decide
whether a channel is available for a user.

**Strategic**

The word "today" in the first sentence implies the near-future variant. More
...
Each `Channel` holds its client, its address lookup, and its own answer to "is
