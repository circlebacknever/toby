...
## Teaching decks, built only when asked

Build a Toby Artifact Style HTML deck (paginated, prev/next, React `.jsx`) only when the user has asked for a produced artifact, deck, or slides. A lesson, walkthrough, or explanation in conversation stays in chat, because toby-explain and toby-learning handle those answers and write them in prose. Once a deck has been asked for, apply artifact copy rules to its copy.

Split the content between the deck and chat.
...
Let the slide count grow with the material, but keep the deck to the fewest slides that still teach the whole concept. If the material needs 18 slides, build 18.
...
**Ink-anchored.** Paper is the default surface. Ink panels interrupt the paper at section dividers, quoted measurements, and named decisions. The artifact is mostly paper. Its ink panels show the points where it states a judgment. It works well for reference decks with multiple sections.
...
```css
/* Surfaces */
--toby-paper:        #fdfaf1;  /* The primary surface is a bright off-white with slight warmth. */
...
--toby-accent:       #c44e3f;  /* Consequence red is only for risk, breach, and finality. */
...
--surface-card:       #fffefa;  /* This is an alias of paper-3 for card backgrounds. */
...
--code-kw:            #8ec8d8;  /* Keywords use a lifted info teal. */
--code-num:           #e8a48e;  /* Numbers use warm coral. */
--code-str:           #a8c98a;  /* Strings use a lifted green. */
--code-com:           #7c7568;  /* Comments are muted and italic. */
```
...
- **Casing:** Use UPPERCASE eyebrows (tracked 0.12em) for section labels and sentence case for headings and body. Never use Title Case. Write tokens in all-lowercase (`$toby-paper`). Set values, IDs, and coordinates in mono.
...
/* Small radii make the system look serious. */
...
Each artifact gets its own logo or wordmark.
...
### pptx slide decks
- See `references/decks.md`. The pptx skill handles the file mechanics. Toby Artifact defines the colors, fonts, sizes, padding, and slide structure.
...
**Slide and chart titles state the claim in a plain, descriptive sentence.** A section heading is a one- or two-word label or a phrase that says what the section covers. The body explains or qualifies the claim. Put caveats next to the claim. Do not move a caveat into a footnote.

**Give every number a unit, because a number without a unit is a defect.**

**Diagram text follows the same rules.** A diagram title is a plain sentence that says what the diagram shows. A node label names the part in the reader's words. An arrow label is a literal verb, such as reads, calls, writes, or returns. An arrow labelled `feeds`, `lives in`, or `talks to` fails.
...
Toby's global voice rules set the register. This skill covers artifact structure, visual tokens, density, copy tests, and component patterns.
