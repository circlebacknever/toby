...
Make changed code simpler to read and keep behavior identical. A lateral rewrite that only swaps one style for another is churn. The skill exists to prevent churn, so a diff shipped to look productive is the failure.

## Disposition

- Precision over recall. A missed cleanup costs less than a churning diff or a silent behavior change. When unsure, leave it.
- The win is countable or it isn't a win. Before you touch anything, say which number goes down: lines, branches, state variables, duplicated blocks, or named traps removed. "Clearer," "tidier," "more idiomatic" count nothing down; they're how a lateral rewrite disguises itself.
- Prove behavior is preserved, then change. The burden sits on the edit; the default is to leave the code alone.
...
- Reach for a library only when it replaces a reimplementation in a hazard-prone class: timezones, unicode, retry and backoff, deep clone or equality, schema validation. Outside those, a few lines of plain stdlib stay — saving a line or two isn't worth an import. The library must already be a direct dependency; confirm it in the manifest. Adding a dependency is out of scope — note it as a follow-up.

## Not a simplification

Reordering for taste, a rename that doesn't fix a misleading name, swapping one construct for another of equal length and clarity, splitting or merging expressions with no debugging gain, formatting a tool owns. None of these clear the countable-win bar — leave them.
...
3. An idiom that appears nowhere else in the repo stays out of a cleanup pass — that's a style migration, noted as a follow-up.
...
Mechanical means a pure rename, a dead-code deletion, or a swap where the edge for its class is provably unreachable — and you quote why ("can't be null — typed string, no | null"). Anything where the edge could fire is edge-crossing: it needs a covering test, or you leave it and note the edge that needs one. Assumed equivalence doesn't ship.
...
- Library substitution — error type and shape, ordering and stability, locale and timezone, and precision all match. A change in performance class (linear to quadratic, sync to async) is a behavior change; leave it.
